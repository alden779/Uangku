...
export function createMotionFeedback({reducedMotion=()=>globalThis.matchMedia?.('(prefers-reduced-motion: reduce)').matches??false,random=Math.random}={}){
  ...
  return {play(kind,progress=0){
    clearTimeout(timer);
    const config={save:['celengan','Celengan makin gendut!'],income:['jempol','Dompet dapat amunisi!'],expense:['kaget','Dadah duit, sudah dicatat!'],target:['merayakan','Target tercapai! Impian makin dekat!'],backup:['jempol','Catatan aman di awan!']}[kind];
    if(!config)return;
    const reduced=reducedMotion();layer.dataset.kind=kind;layer.dataset.reduced=String(reduced);
    const count=reduced?0:kind==='target'?24:kind==='save'?6+Math.round(progress*8):kind==='expense'?3:kind==='backup'?4:8;
    let particles='';
    for(let i=0;i<count;i++){
      const x=Math.round((random()-.5)*85),y=Math.round((random()-.5)*65),angle=Math.round((random()-.5)*80),delay=Math.round(i*35);
      const art=kind==='target'?'<i></i>':kind==='expense'?winged:kind==='backup'?(i===0?cloud:'<span class="paper-icon">✓</span>'):i%3===0?note:coin;

...
export class DriveBackup{
  ...
  async authorized(response){
    ...
    try{await this.list();this.emit('Drive tersambung. Perubahan berikutnya akan dicadangkan otomatis. Pilih Backup sekarang untuk mencadangkan data saat ini.')}catch(e){this.emit(e.message)}
  }
  disconnect(){this.generation++;clearTimeout(this.timer);this.token='';this.expires=0;this.pending=null;this.files=[];this.lastFingerprint='';this.state.lastBackup=null;this.emit('Backup otomatis dihentikan. Salinan di Drive tetap ada.')}
  ...
  async flush(force=false){
    ...
    }catch(e){if(generation===this.generation)this.emit(e.message);throw e}finally{this.busy=false;if(generation===this.generation){this.emit(this.state.message);if(this.pending&&this.token&&this.now()<this.expires&&JSON.stringify(this.pending)!==fingerprint){clearTimeout(this.timer);this.timer=setTimeout(()=>this.flush().catch(()=>{}),this.delay)}}}
  }
